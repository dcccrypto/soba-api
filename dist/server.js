...
async function getTokenPrice() {
    try {
        const response = await axios.get(`https://api.solanatracker.io/v1/token/${TOKEN_ADDRESS}/price`, {
            headers: {
...
async function getTokenSupply(connection) {
    try {
...
async function getFounderBalance(connection) {
    try {
        const tokenPubkey = new PublicKey(TOKEN_ADDRESS);
...
async function getHolderCount() {
    try {
        const response = await axios.get(`https://api.helius.xyz/v0/token-metadata/${TOKEN_ADDRESS}?api-key=${HELIUS_API_KEY}`);
